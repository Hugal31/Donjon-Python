...
import pygame
from pygame.locals import placerCoffre
...
def changeValue(pos, pas):
    ...
    x = (pos[0] - 32)//32
    y = (pos[1] - 32)//32
    if x >= 0 and x <= 12 and y >=0 and y <=6 :
        grilleTiles[y][x].valeur = (grilleTiles[y][x].valeur+pas)%6
...
def changerPack():
    global texturePack
    texturePack = input('Entrez un nom de texture pack : ')
    try:
        texutre(texturePack)
        rafraichir()
    except : None
...
while continuer:
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            elif event.key == K_t : changerPack()
